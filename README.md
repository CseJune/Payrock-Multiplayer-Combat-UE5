...
void APRCharacter::ServerSetAiming_Implementation(bool bNewAimimng)
{
...
```
클라이언트 입력으로 조준 상태가 시작되면 Server RPC를 통해 서버 상태를 먼저 동기화하고, 이후 OnRep_IsAiming()에서 회전방식을 전환하도록 구성했습니다.
이를 통해 멀티플레이 환경에서도 조준 상태에 따른 캐릭터 회전 방식이 일관되게 적용되도록 구현했습니다.
...
```
점프 입력은 서버에서 처리하도록 구성하고, GAS Gameplay Effect를 사용해 마나 비용을 적용한 뒤 실제 점프를 실행하도록 구현했습니다.
또한 SetJustJumped(true)를 통해 점프 직후 애니메이션 연동에 필요한 상태까지 함께 반영하도록 설계했습니다.
